...
public final class CreditExchange {
    ...
    public static Result redeemAll(ServerPlayer player, Item item,
                                   EconomyManager economy, CreditStorage storage) {
        long unitValue = unitValueOf(item, economy);
        if (unitValue <= 0) {
            return Result.failed("tablegames.exchange.not_convertible");
        }

        long balance = storage.balanceOf(player.getUUID());
        long affordable = balance / unitValue;
        if (affordable <= 0) {
            return Result.shortOf("tablegames.exchange.cannot_afford_one", 0, unitValue);
        }
        ...
        return commit(player, item, count, count * unitValue, storage);
    }

    /**
     * Buys back an exact number of an item.
     * <p>
    ...
    public static Result redeemExactly(ServerPlayer player, Item item, long requested,
        ...
        return commit(player, item, requested, cost, storage);
    }

    private static long unitValueOf(Item item, EconomyManager economy) {
        return economy.table().valueOf(ItemIds.idOf(item)).orElse(0L);
    }
    ...
    private static Result commit(ServerPlayer player, Item item, long count,
                                 long cost, CreditStorage storage) {
